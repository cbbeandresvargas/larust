...
- **Models** (`src/models/`): plain structs deriving `FromRow` (+ `Serialize`/`Deserialize` as needed) that mirror a table's columns exactly, since `query_as::<_, T>` maps positionally/by-name from the SELECT. `id` fields are `String`. Re-exported through `models/mod.rs`. `User::password_hash` is `#[serde(skip_serializing)]` — never let it leak into a JSON response.
